...
public class HashMapSource1 {
    public static void main(String[] args) {
        ...
        2. 执行 put 调用 hash 方法, 计算 key 的 hash值 (h = key.hashCode()) ^ (h >>> 16)
                ...
                final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                   boolean evict) {
                    ...
                    if ((tab = table) == null || (n = tab.length) == 0)
                        n = (tab = resize()).length;
                    //取出 hash 值对应的 table 的索引位置 Node, 如果为 null, 就直接把加入的 k-v,
                    //创建成一个 Node, 加入该位置即可
                    if ((p = tab[i = (n - 1) & hash]) == null)
                        tab[i] = newNode(hash, key, value, null);
                    else {
                        Node<K,V> e; K k; //辅助变量
                        //如果 table 的索引位置的 key 的 hash 和 新的key 的 hash 值相同,
                        //并 满足(table 现有的结点的 key 和准备添加的 key 是同一个对象 || equals 返回真),
                        //就认为不能加入新的 k-v
                        if (p.hash == hash &&
                            ((k = p.key) == key || (key != null && key.equals(k))))
                            e = p;
                        else if (p instanceof TreeNode) //如果当前的 table 的已有的 Node 是红黑树,就按红黑树的方式处
